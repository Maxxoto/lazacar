...
include '../../koneksi.php';
$id_beli = $_GET['id'];
$hasil = mysql_query("SELECT * from tb_pembeli where id_beli='$id_beli' order by id_beli asc") or die (mysql_error());
$rows = mysql_num_rows($hasil);

if($rows < 1){
	echo "<script> alert('Sebelum Mencetak , Silahkan Klik Konfirmasi terlebih dahulu') ;
	document.location.href='../users/transaksi.php' </script>";
}else{
